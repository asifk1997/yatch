...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool) *Replica {
	...
	for i := 0; i < r.N; i++ {
		r.InstanceSpace[i] = make([]*Instance, 100)
		r.crtInstance[i] = 0
		r.ExecedUpTo[i] = -1
...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case <-r.OnClientConnect:
			log.Printf("weird %d; conflicted %d; slow %d; happy %d;MsgCount %d\n", weird, conflicted, slow, happy, r.MsgCount)
...
func (r *Replica) bcastPreAccept(replica int32, instance int32, ballot int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	pa.LeaderId = r.Id
	dlog.Println("pa.LeaderId", pa.LeaderId)
	pa.Replica = replica
	dlog.Println("pa.Replica", pa.Replica)
	pa.Instance = instance
	dlog.Println("pa.Instance", pa.Instance)
	pa.Ballot = ballot
	dlog.Println("pa.Ballot", pa.Ballot)
	pa.Command = cmds
	dlog.Println("pa.Command", pa.Command)
	pa.Seq = seq
	dlog.Println("pa.Seq", pa.Seq)
	pa.Deps = deps
	dlog.Println("pa.Deps", pa.Deps)
	args := &pa
...
func (r *Replica) bcastAccept(replica int32, instance int32, ballot int32, count int32, seq int32, deps [DS]int32) {
	dlog.Println("bcastAccept", "replica", replica, "instance", instance, "ballot", ballot)
	defer func() {
		if err := recover(); err != nil {
...
func (r *Replica) updateConflicts(cmds []state.Command, replica int32, instance int32, seq int32) {
	//dlog.Println("updateConflicts", "cmds", cmds, "replica", replica, "instance", instance, "seq", seq)

	for i := 0; i < len(cmds); i++ {
		if d, present := r.conflicts[replica][cmds[i].K]; present {
			//dlog.Println("hashtable1", "replica", replica, "cmds", cmds[i].K, "instance", instance, "d", d, "present", present)
			if d < instance { // if d < instance // a way to order or break the cycle
				//dlog.Println("hashtable2", "replica", replica, "cmds", cmds[i].K, "instance", instance, "d", d, "present", present)
				r.conflicts[replica][cmds[i].K] = instance
			}
		} else {
			//dlog.Println("putting this value to hashtable", "replica", replica, "cmds", cmds[i].K, "instance", instance)
			r.conflicts[replica][cmds[i].K] = instance
		}
...
func (r *Replica) visualise() {
	r.visualiseConflicts()
	r.visualiseInstances()
}

func (r *Replica) visualiseConflicts() {
	dlog.Println("-----------------visualiseConflicts")
	for q := 0; q < r.N; q++ {
		dlog.Println("q", q)
		dlog.Println(r.conflicts[q])
	}

}

func (r *Replica) visualiseInstances() {
	dlog.Println("-----------------visualiseInstances")
	for q := 0; q < r.N; q++ {
		dlog.Println("q", q)
		for i := 0; i < 100; i++ {
			...
			dlog.Println("i", i)
			dlog.Println("cmds", r.InstanceSpace[q][i].Cmds)
			dlog.Println("ballot", r.InstanceSpace[q][i].ballot)
			dlog.Println("status", r.InstanceSpace[q][i].Status)
			dlog.Println("seq", r.InstanceSpace[q][i].Seq)
			dlog.Println("deps", r.InstanceSpace[q][i].Deps)
			dlog.Println("index", r.InstanceSpace[q][i].Index)

		}
	}
}

func (r *Replica) visualiseInstances2() {
	dlog.Println("visualiseInstances")
	for i := 0; i < 100; i++ {
		for q := 0; q < r.N; q++ {
			if r.InstanceSpace[q][i] == nil {
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	dlog.Println("handlePropose")
	dlog.Println(propose.Command)
	dlog.Println(propose.CommandId)
	dlog.Println(propose.Timestamp)
	...
	dlog.Println("time taken handle propose", elapsed)
...
func (r *Replica) startPhase1(replica int32, instance int32, ballot int32, proposals []*genericsmr.Propose, cmds []state.Command, batchSize int) {
	...
	dlog.Println("replica", replica, "instance", instance, "ballot", ballot, "proposals", proposals, "cmds", cmds, "batchsize", batchSize)
	seq := int32(0)
	var deps [DS]int32
	for q := 0; q < r.N; q++ {
		deps[q] = -1
	...
	r.InstanceSpace[r.Id][instance] = &Instance{
		...
		nil}
	//dlog.Println("calling visualiseInstances")
	//r.visualiseInstances()
	//dlog.Println("calling updateConflicts")
	r.updateConflicts(cmds, r.Id, instance, seq)
	//dlog.Println("calling visualise")
	//r.visualise()
	if seq >= r.maxSeq {
		r.maxSeq = seq + 1
	}
	//dlog.Println("recordInstanceMetadata")
	r.recordInstanceMetadata(r.InstanceSpace[r.Id][instance])
	r.recordCommands(cmds)
	r.sync()
	dlog.Println("Sending PreAccept1")
	elapsed := time.Since(start)
	dlog.Println("time taken in startPhase1", elapsed)
...
func (r *Replica) handlePreAcceptReply(pareply *epaxosproto.PreAcceptReply) {
	...
	if inst.lb.preAcceptOKs >= r.N/2 && inst.lb.allEqual && allCommitted && isInitialBallot(inst.ballot) {
		...
		dlog.Println("time taken handlePreAcceptReply", end)
...
func (r *Replica) handlePreAcceptOK(pareply *epaxosproto.PreAcceptOK) {
	dlog.Printf("Handling PreAcceptOK reply\n")
	start := time.Now()
...
func (r *Replica) handleAccept(accept *epaxosproto.Accept) {
	...
	dlog.Println("time taken handleAccept", end)
...
func (r *Replica) handleCommit(commit *epaxosproto.Commit) {
	dlog.Println("handleCommit", commit.Replica, commit.Instance)
	start := time.Now()
	...
	dlog.Println("time taken in handleCommit", end, "MsgCount", r.MsgCount)
}

func (r *Replica) handleCommitShort(commit *epaxosproto.CommitShort) {
	dlog.Println("handleCommitShort", commit.Replica, commit.Instance)
	start := time.Now()
	inst := r.InstanceSpace[commit.Replica][commit.Instance]
	...
	dlog.Println("time taken in handleCommit", end)
}
