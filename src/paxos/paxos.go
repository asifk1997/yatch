...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case <-clockChan:
			...
			//dlog.Println("//activate the new proposals channel")
			onOffProposeChan = r.ProposeChan
			break
		...
		case propose := <-onOffProposeChan:
			...
			dlog.Println("propose", propose.Command)
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	batchSize := len(r.ProposeChan) + 1 //here comes the data from propose channel //asif
	...
	if r.defaultBallot == -1 {
		...
		r.instanceSpace[instNo] = &Instance{
			...
			PREPARED,
			&LeaderBookkeeping{proposals, 0, 0, 0, 0}}
		dlog.Println("r.instanceSpace", r.instanceSpace[instNo])
...
func (r *Replica) executeCommands() {
	log.Println("inside executeCommands")
	i := int32(0)
	for !r.Shutdown {
		executed := false
