...
func initReplica() *genericsmr.Replica {
	peers := make([]string, 3)
	r := &Replica{genericsmr.NewReplica(0, peers, false, false, false),
		make(chan *epaxosproto.Propose, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.Prepare, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.PreAccept, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.PreAccept, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.Accept, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.Commit, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.PrepareReply, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.PreAcceptReply, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.PreAcceptOK, genericsmr.CHAN_BUFFER_SIZE),
		make(chan *epaxosproto.AcceptReply, genericsmr.CHAN_BUFFER_SIZE),
		make([][]*Instance, 3),
		make([]int32, 3),
		false,
		0,
