...
var doYatch *bool = flag.Bool("y", false, "Use Yatch as the replication protocol. Defaults to false.")
...
func main() {
	...
	if *doEpaxos {
		...
		log.Println("Starting Yatch replica...")
		rep := yatch.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *beacon, *durable)
		rpc.Register(rep)
	} else {
